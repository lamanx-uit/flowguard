...
from app.exception_handlers import requestValidationError, openAIError, genericExceptionHandler
...
async def generic_exception_handler(request: Request, exc: Exception):
    return await genericExceptionHandler(request, exc)

@app.exception_handler(OpenAIError)
async def openai_exception_handler(request: Request, exc: OpenAIError):
    return await openAIError(request, exc)